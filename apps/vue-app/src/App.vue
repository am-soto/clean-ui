...
const colors = ref(Object.keys(Colors).filter((item) => item !== "white" && item !== "black")
)
        ...
        <div slot="left-bar">
            ...
            <ul v-if="showColors" class="pt-4">
                <li v-for="item in colors">
                    <button :key="item.id" type="button" class="rounded-full w-7 h-7 animate__animated animate__fadeInDown"
                        :class="`bg-${item}`" @click="createTask(item)" />
                </li>
            </ul>
        </div>
        <div slot="main">
            ...
            <ul className="flex flex-wrap gap-4 py-2 overflow-auto" ref="parent">
                <li v-for="task in tasks">
                    <Card :task="task" :focus="focusNew" @delete.once="deleteTask(task)" @update="updateTask" />
                </li>
            </ul>
        </div>
    </home-layout>
